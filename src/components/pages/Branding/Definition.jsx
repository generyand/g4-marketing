    ...
    <SubTopicContainer>
      ...
      {brandingData.map((item, index) => (
        <div key={index} className="flex flex-col mb-8 md:mb-12">
          ...
          <div className="flex flex-col items-center justify-center gap-2 p-4 bg-blue-200 rounded-lg md:gap-8 sm:p-8 md:p-10 sm:items-start lg:flex-row">
            <img
              src={item.image}
              alt={`${item.title} illustration`}
              className="mx-auto mb-4 text-xs sm:w-64 sm:mb-0"
            ...
            /> 
            <p className="self-center text-gray-800">{item.description}</p>
          </div>
        </div>
      ))}
    </SubTopicContainer>
